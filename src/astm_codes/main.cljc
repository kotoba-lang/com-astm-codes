(ns astm-codes.main
  "Kotodama WASM entrypoint for the Astm_codes clean-room actor (L4) — Clojure port.
  ...
  facts (`astm_codes.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "astm_codes")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"astm_codes.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "astm_codes-compat" :tier tier :entities entities} 200])
...
(defn start! [] :astm_codes-compat/ready)
